...
def main():
    print("Pracujemy na " + network.show_active())
    advanced_collectible = AdvancedCollectible[len(AdvancedCollectible) - 1]
    number_of_tokens = advanced_collectible.tokenCounter()
    print("Liczba tokenow jakie stworzyles to {}".format(number_of_tokens))
...
def write_metadata(number_of_tokens, nft_contract):
    for token_id in range(number_of_tokens):
        ...
        print("./metadata/{}/".format(network.show_active()) + str(token_id)
            + "-" + breed + ".json")
        if Path(metadata_file_name).exists():
            ...
            collectible_metadata["description"] = "An adorable {} pub!".format(
